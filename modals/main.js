...
function fadeIn() {
  TweenMax.to(modal, .2, { autoAlpha: 1 });
}
...
function slideIn() {
  TweenMax.to(modal, .5, { autoAlpha: 1 });
  TweenMax.from(modalBox, .5, { autoAlpha: 0, top: -300 })
}
...
function close() {
  TweenMax.set([modal, modalClose, modalBox, modalHeader, modalTitle], { clearProps: 'all' }) // limpa todas as propriedades que estão sendo utilizadas no momento
  TweenMax.to(modal, .2, { autoAlpha: 0 })
}
